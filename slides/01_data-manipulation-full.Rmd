...
--
```{r, eval = F}
imdb[imdb$Duration == "104 min", "Name"]
...
```{r}
  library(stringr)
  imdb %>% 
    mutate(comedy = if_else(str_detect(Genre, "Comedy"),
                           1,
                           0),
           drama = if_else(str_detect(Genre, "Drama"),
                           1,
                           0)) %>% 
    select(comedy, drama, Genre) %>% 
    summarise(share_comedy = mean(comedy),
              share_drama = mean(drama))
```
...
```{r, eval = F}
  imdb %>% 
    ... %>% 
    summarise(n = n(),
              share_comedy = sum(comedy) / n, #<<
              share_drama = sum(drama) / n) #<< 
```

---
### ANSWERS
- Create a variable *category* that classifies movies into short or long depending on whether or not they exceed 40 min., and create a separate ranking by category. <br>
